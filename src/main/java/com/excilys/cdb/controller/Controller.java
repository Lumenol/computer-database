...
import com.excilys.cdb.mapper.dto.CreateCompanyDTOToCompanyMapper;
...
import com.excilys.cdb.validator.CreateCompanyDTOValidator;
...
public class Controller {
	...
	try {
	    CreateCompanyDTOValidator.getInstance().check(dto);
	    companyService.create(CreateCompanyDTOToCompanyMapper.getInstance().map(dto));
	} catch (ValidationException e) {
	    logger.warn("createCompany(" + dto + ")", e);
	    throw e;
	} catch (RuntimeException e) {
	    logger.warn("createCompany(" + dto + ")", e);
	    throw new ControllerException();
